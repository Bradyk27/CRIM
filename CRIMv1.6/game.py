...
def find_data_file(filename,normalpath):
    if getattr(sys, 'frozen', False):
        ...
        datadir = os.path.dirname(normalpath)
...
def Sound():
    pygame.init()
    pygame.mixer.music.load(find_data_file('test.mp3','test.mp3'))
    pygame.mixer.music.play()

def StartButton():
    global window
    window = Tk()
    window.wm_title("CRIM v1.6")
    photo = PhotoImage(file =  find_data_file("crim.gif", "crim.gif"))
    photo = photo.subsample(2, 2)
    button = Button(window,fg = "white", bg = "black" ,height = 500, width = 200, text = "Start Game", command = play, highlightbackground = 'black')
